...
import type { TenantPkgForm, TenantPkgVO } from "@/common/apis/system/tenantPackage/types"
import { useDevice } from "@@/composables/useDevice.ts"
import { formatDateTime } from "@@/utils"
import { CirclePlus, RefreshRight } from "@element-plus/icons-vue"
import { ref } from "vue"
import { changeSysUserStatusApi } from "@/common/apis/system/user"
 ...
 */
// #region defineModel
const tableData = defineModel<TenantPkgVO[]>("tableData", { required: true })
...
export interface EmitEvents {
  openAddDialog: []
  handleDelete: [rows: TenantPkgForm[]]
  handleExport: []
...
const handleDelete = (rows: TenantPkgForm[]) => emit("handleDelete", rows)
...
const selectedRows = ref<TenantPkgForm[]>([])

const handleSelectionChange = (val: TenantPkgForm[]) => (selectedRows.value = val)

async function handleStatusChange(row: TenantPkgVO) {
  const text = row.status === "0" ? "启用" : "停用"
  try {
    await ElMessageBox.confirm(`确认要"${text}""${row.packageName}"吗?`, "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
    })
    await changeSysUserStatusApi(row.packageId, row.status)
      ...
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="status" label="状态" align="center">
          <template #default="scope">
            <el-switch v-model="scope.row.status" active-value="0" inactive-value="1" v-hasPermi="['system:tenantPackage:edit']" @change="handleStatusChange(scope.row)" />
          </template>
        </el-table-column>
